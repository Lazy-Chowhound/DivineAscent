...
​	*MySql*索引底层采用*B*+树的数据结构，仅在叶子节点存储数据，并且从左到右串联起来便于范围查询。这样的结构导致复合索引的查询符合“最左原则”，也就是当建立（*a*,*b*,*c*）复合索引时，查询条件中必须有 *a* 才能走索引，如*a*,*ab*,*ac*,*abc* ;但是*b*,*bc*,*c*都不能走索引。
...
由于旧数据并不真正的删除，所以必须对这些数据进行清理，*innodb* 会开启一个后台线程执行清理工作，具体的规则是将删除版本号小于当前事务版本的行删除，这个过程叫做 *purge*。
...
一个表有 *id* 为1，2，3，5，6，9行数据，执行如下 *sql* 语句
